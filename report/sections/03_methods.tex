...
\begin{enumerate}
    ...
    (default hyperparameters) was trained on the patch-level mean features.
...
No hyperparameter tuning was performed for the baseline, ensuring a fair 
comparison with the GNN-SDM under equivalent pseudo-absence conditions.
...
PyTorch Geometric \citep{Fey2019}. The model consists of a stack of 
\texttt{SAGEConv} layers with mean aggregation, each followed by LeakyReLU 
activation and dropout ($p = 0.2$), and a final linear layer with sigmoid output 
producing a habitat-suitability score in $[0, 1]$ for every patch.
