...
A natural question arises as to whether the circuit-theory step is redundant 
...
alternative paths exist. Conservation action is most impactful at patches that 
score highly on \emph{both} dimensions---places that are simultaneously good 
habitat and vital connectivity links.
...
The gap analysis targets threatened species (IUCN categories NT, VU, EN, CR) 
that have both GNN suitability scores and sufficient spatial structure for 
circuit analysis. Two filters are applied:

\begin{enumerate}
    \item \textbf{Non-native exclusion.} Species whose IUCN threat status 
    refers to their native range outside Switzerland (e.g.\ \textit{Ginkgo 
    biloba}, \textit{Sequoiadendron giganteum}) are excluded, as their 
    occurrences in Switzerland represent planted individuals in parks and 
    gardens rather than populations requiring habitat connectivity.

    \item \textbf{Minimum connectivity.} Species whose observation patches 
    form fewer than 2 connected components of size $\geq 3$ are excluded, 
    as the circuit-theory framework requires at least one source--ground 
    pair.
\end{enumerate}
...
Per-species current flow values are normalised to $[0, 1]$ using the 99th 
percentile as ceiling:
\begin{equation}
    \tilde{c}_{v} = \min\!\left( 1,\; 
        \frac{c_v}{P_{99}(c)} 
    \right)
    \label{eq:normalisation}
\end{equation}
This percentile-based approach is more robust than max-normalisation, which 
is sensitive to single extreme pinch-point patches that would compress all 
other values toward zero.

Species are weighted by IUCN threat level (CR\,=\,4, EN\,=\,3, VU\,=\,2, 
NT\,=\,1) to reflect conservation urgency. The multi-species priority surface 
is then computed as:
\begin{equation}
    P_v = \frac{1}{2}\left( 
    ...
    \label{eq:priority}
\end{equation}
where $w_s$ is the IUCN weight of species $s$, $\hat{y}_{v,s}$ the GNN 
suitability, and $\tilde{c}_{v,s}$ the normalised current flow at patch $v$.
