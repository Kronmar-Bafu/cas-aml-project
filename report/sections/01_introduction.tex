...
Switzerland, despite its relatively small area, harbours a unique plant 
diversity owing to its pronounced altitudinal gradients and mosaic of biogeographic 
regions. Nevertheless, a third of all assessed species and half of all habitat 
...
\citep{InfoFlora2016}. The main drivers, such as soil sealing, landscape fragmentation, 
...
spatially explicit predictions of where species can persist, and which 
landscape elements link suitable habitat patches into functional networks.
Species distribution models (SDMs) have become a tool in conservation efforts,
relating georeferenced occurrence records to environmental 
...
occurrence as an independent sample drawn from an environmental niche. This 
point-based paradigm ignores the spatial structure of landscapes: two 
patches with identical environmental conditions may differ in 
...
habitat patches, such an omission can lead to 
misleading suitability estimates and suboptimal conservation recommendations.

Graph Neural Networks (GNNs) offer a way to incorporate landscape 
structure into SDMs, by representing the landscape as a graph, where nodes are 
ecologically homogeneous patches and edges encode spatial adjacency. 
\citet{Wu2025} introduced the SOM-GNN-SDM framework, 
demonstrating that a GraphSAGE architecture \citep{Hamilton2017} trained on 
landscape graphs outperforms traditional SDMs for virtual and real
...
that are spatially coherent. Additionally, the graph structure enables conservation
applications beyond suitability mapping. The Swiss federal government has identified 
...
In this work, we adapt the SOM-GNN-SDM framework to Swiss vascular flora at 30\,m 
resolution, presenting a finer grain than the original continental-scale 
...
per-species GraphSAGE models for 2,696 qualifying plant species,
and benchmark against a Random Forest baseline. We further apply circuit-theory connectivity 
...
\begin{enumerate}[nosep]
  ...
  \item Can the graph structure be used to identify spatial gaps in the protected-area network where additional conservation measures would be most beneficial?
\end{enumerate}
